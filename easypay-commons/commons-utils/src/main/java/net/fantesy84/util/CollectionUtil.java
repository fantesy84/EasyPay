...
public abstract class CollectionUtil {
	 ...
	 * @since 3.1
	 */
	public static <K, V> MixValMap<K, V> toMultiValueMap(Map<K, List<V>> map) {
		return new MixValMapAdapter<K, V>(map);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <K, V> MixValMap<K, V> unmodifiableMultiValueMap(MixValMap<? extends K, ? extends V> map) {
	...
	private static class MixValMapAdapter<K, V> implements MixValMap<K, V>, Serializable {

		private final Map<K, List<V>> map;

		public MixValMapAdapter(Map<K, List<V>> map) {
			Assert.assertNotNull("'map' must not be null", map);
			this.map = map;
		}
